...
[ -f "$top_src" ] || { echo "FATAL: missing $top_src (run equilibrium pipeline first)"; exit 2; }
...
cp -f "$RELAX"/*.itp "$WORK"/ 2>/dev/null || true
...
CUDA_VISIBLE_DEVICES="$gpu" "$GMX" mdrun -deffnm em -ntmpi 1 -nb gpu >prep_em.log 2>&1
...
CUDA_VISIBLE_DEVICES="$gpu" "$GMX" mdrun -deffnm nvt -ntmpi 1 -ntomp 8 -nb gpu -pme gpu >prep_nvt.log 2>&1
...
CUDA_VISIBLE_DEVICES="$gpu" "$GMX" mdrun -deffnm npt -ntmpi 1 -ntomp 8 -nb gpu -pme gpu >prep_npt.log 2>&1
