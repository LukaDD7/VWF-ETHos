...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --md-tag) md_tag="$2"; shift 2 ;;
    -h|--help)
      echo "Usage: bash scripts/pipeline/run_md_variant_direct.sh --variant V --gpu 0 [--system a1_gpiba] [--md-tag md_a1_gpiba]"
      exit 0 ;;
    *) echo "[WARN] unknown arg: $1"; shift ;;
  esac
done

[ -n "$variant" ] || { echo "missing --variant"; exit 2; }
[ -n "$gpu" ] || { echo "missing --gpu"; exit 2; }
[ -n "$md_tag" ] || md_tag="md_${system}"
...
[ -f "$start" ] || { echo "missing start for $variant: $relax"; exit 2; }
[ -f "$relax/topol.top" ] || { echo "missing topol for $variant: $relax"; exit 2; }
...
echo "[$(date '+%F %T')] $variant direct start system=$system md_tag=$md_tag gpu=$gpu pinoffset=$pinoffset start=$start"
